    ...
    <c:forEach items="${productList}" var="product" varStatus="status">
        <tr>
            ...
            <td>
                ...
                <c:if test="${product.loai == type.maCategory}"> ${type.tenCategory} </c:if>
                </c:forEach>
            <td>
                <button id="edit"  type="button" class="btn btn-primary" data-toggle="modal" data-target="#editForm">
                    <a style="color: white;text-decoration: none" href="/product?action=edit&id=${product.tenSanPham}"> Edit </a>
                </button>
            </td>
            <td><%--                onclick="showDelete('${cus.customerId}','${cus.customerName}')"--%>
                <button  type="button" class="btn btn-primary "
                         data-toggle="modal" data-target="#deleteForm" onclick="showDelete('${product.tenSanPham}','${product.tenSanPham}')">Delete</button>
            </td>
        </tr>
