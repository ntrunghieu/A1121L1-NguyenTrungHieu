...
<table id="tableStudent" class="table table-area" style="width: 80%">
    ...
    <tbody>
    <c:forEach items="${sachList}" var="sach" varStatus="status">
        <tr>
            ...
            <td>
                <button id="edit"  type="button" class="btn btn-primary" data-toggle="modal" data-target="#editForm">
                    <a style="color: white;text-decoration: none" href="/book?action=muon&id=${sach.maSach}"> Mượn </a>
                </button>
            </td>
            <td hidden>
                    <%--                onclick="showDelete('${cus.customerId}','${cus.customerName}')"--%>
                <button hidden type="button" class="btn btn-primary "
                        data-toggle="modal" data-target="#deleteForm" onclick="showDelete('${sach.maSach}','${sach.tenSach}')">Delete</button>
            </td>
        </tr>
